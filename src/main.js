...
const createWindow = () => {
  ...
  //-----------------------------------------------------------------
  //添加主菜单（在开发测试时会有一个默认菜单，但打包后这个菜单是没有的，需要自己增加）
  const menu = Menu.buildFromTemplate(appMenuTemplate); //从模板创建主菜单
  //在file菜单下添加名为new的子菜单
  menu.items[0].submenu.append(new MenuItem({  //menu.items获取的是主菜单一级菜单的菜单数组，menu.items[0]在这里就是第1个File菜单对象，在其子菜单submenu中添加新的子菜单
    label:"New",
    click(){
  ...
  Menu.setApplicationMenu(menu);  //注意:这个代码要放到菜单添加完成之后，否则会造成新新增菜单的快捷键无效
